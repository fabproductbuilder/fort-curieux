-- Seed Culture / Histoire : événements, personnages, dates et périodes clés.
-- Cette migration ajoute uniquement du contenu read-only dans les tables Culture existantes.
-- Elle est idempotente : les items et prompts déjà présents ne sont pas réinsérés.
...
with source_events as (
	...
	select
		...
		source_items.sort_order::integer
	from source_items
	where not exists (
		select 1
		from public.culture_items existing_items
		where existing_items.category = 'history'
	...
	select inserted_items.id, inserted_items.collection, inserted_items.title
	from inserted_items
	union all
	...
	where culture_items.category = 'history'
		and not exists (
			select 1
			from inserted_items
			where inserted_items.id = culture_items.id
		)
), source_prompts as (
	select
		'historical_events'::text as collection,
...
-- select count(*) from public.culture_items where category = 'history' and collection in ('historical_events', 'historical_figures', 'historical_dates');
-- select count(*) from public.culture_prompts join public.culture_items on culture_items.id = culture_prompts.item_id where culture_items.category = 'history' and culture_items.collection in ('historical_events', 'historical_figures', 'historical_dates');
